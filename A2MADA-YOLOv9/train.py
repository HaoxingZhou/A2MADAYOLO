...
from utils.dataloaders import create_dataloader, create_target_dataloader
...
from utils.loss_tal import ComputeLoss, ComputeLossDA
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    target_dataloader, target_dataset = create_target_dataloader(target_train_path,
                                                                 imgsz,
                                                                 batch_size // WORLD_SIZE,
                                                                 gs,
                                                                 single_cls,
    ...
    compute_loss = ComputeLossDA(model)  # init loss class
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK != -1:
            ...
            target_dataloader.sampler.set_epoch(epoch)  # 设置目标域数据的epoch
        LOGGER.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'da_loss', 'total', 'labels', 'img_size'))
        # if RANK in {-1, 0}:
        #     pbar = tqdm(pbar, total=nb, bar_format=TQDM_BAR_FORMAT)  # progress bar

        # 合并数据加载器
        combined_dataloader = zip(train_loader, target_dataloader)
        if RANK in [-1, 0]:
            pbar = tqdm(enumerate(combined_dataloader), total=len(train_loader))  # 使用单个进度条
        ...
        for i, data in pbar:
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                loss, loss_items = compute_loss(pred, targets_src.to(device), da_loss=da_loss)
            ...
            # Log
            if RANK in {-1, 0}:
                mloss = torch.zeros(4, device=device)  # 初始化为四维张量以匹配 loss_items
                mloss = (mloss * i + loss_items) / (i + 1)  # update mean losses
                mem = '%.3gG' % (torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0)  # (GB)
                # 's' 字符串中有 7 个 '%10.4g' 对应 mloss 的 4 个值和 2 个额外的值
                s = ('%10s' * 2 + '%10.4g' * 6) % (
                    '%g/%g' % (epoch, epochs - 1), mem, *mloss, targets_src.shape[0], imgs_src.shape[-1])
                pbar.set_description(s)
                callbacks.run('on_train_batch_end', model, ni, imgs_src, targets_src, paths_src,
                              mloss.tolist())  # 将 mloss 转换为列表
