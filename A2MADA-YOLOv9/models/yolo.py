...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, DDetect, Segment)):
            ...
            outputs_src = forward(torch.zeros(1, ch, s, s))[0]  # Only use imgs_src
            m.stride = torch.tensor([s / x.shape[-2] for x in outputs_src])  # forward
    ...
    def forward(self, imgs_src, imgs_tgt=None, augment=False, profile=False, visualize=False):
        if imgs_tgt is None:
            imgs_tgt = imgs_src
        # if augment:
        #     return self._forward_augment(x)  # augmented inference, None
        imgs_src, captured_outputs_src = self._forward_once(imgs_src, profile, visualize)  # single-scale inference, train
        imgs_tgt, captured_outputs_tgt = self._forward_once(imgs_tgt, profile, visualize)
        # # 打印每个特征图的形状
        # for i, output_list in enumerate(imgs_src):
        #     for j, tensor in enumerate(output_list):
        #         print(f"Output shape [{i}][{j}]:", tensor.shape)
        # 打印返回值的形式
        # print("imgs_src type:", type(imgs_src))
        # print("imgs_src length:", len(imgs_src))
        # print("imgs_src[0] type:", type(imgs_src[0]))
        # print("imgs_src[0][0] type:", type(imgs_src[0][0]))
        # print("imgs_src[0][0] shape:", imgs_src[0][0].shape)
