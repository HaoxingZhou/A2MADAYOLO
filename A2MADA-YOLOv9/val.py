...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        with dt[1]:
            if compute_loss:
                outputs, captured_outputs_src, captured_outputs_tgt = model(im)
                preds, train_out = outputs
            else:
                outputs, captured_outputs_src, captured_outputs_tgt = model(im, augment=augment)
                preds, train_out = outputs

        # # 检查 train_out 中每个元素的形状
        # if isinstance(train_out, list):
        #     print("train_out is a list.")
        #     for i, tensor in enumerate(train_out):
        #         print(f"train_out[{i}] shape: {tensor.shape}")
        # else:
        ...
        # Loss
        if compute_loss:
            loss += compute_loss(train_out, targets, training=False)[1]  # box, obj, cls
