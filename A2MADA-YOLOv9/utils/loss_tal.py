...
class ComputeLossDA:
    ...
    def __call__(self, p, targets, img=None, epoch=0, da_loss=None, training=True):
        # 初始化损失张量，根据是否训练阶段决定长度
        loss_length = 4 if training else 3
        loss = torch.zeros(loss_length, device=self.device)  # box, cls, dfl, da_loss（如果在训练阶段）
        feats = p[1] if isinstance(p, tuple) else p
        pred_distri, pred_scores = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats], 2).split(
            (self.reg_max * 4, self.nc), 1)
        pred_scores = pred_scores.permute(0, 2, 1).contiguous()
        ...
        loss[1] *= 0.5  # cls gain
        loss[2] *= 1.5  # dfl gain
        if training and da_loss is not None:
            loss[3] = 10 * da_loss
        ...
        #
        # # 检查 loss 是否为张量，如果不是，则进一步调查
        # if not isinstance(loss, torch.Tensor):
        #     print("loss is not a tensor. Investigating further...")
        #     if isinstance(loss, list) or isinstance(loss, tuple):
        #         for i, l in enumerate(loss):
        #             print(f"loss[{i}]: {l}, shape: {l.shape}")
