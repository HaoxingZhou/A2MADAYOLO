...
class ComputeLossDA:
    ...
    def bbox_decode(self, anchor_points, pred_dist):
        if self.use_dfl:
            ...
            # pred_dist = (pred_dist.view(b, a, c // 4, 4).softmax(2) * self.proj.type(pred_dist.dtype).view(1, 1, -1, 1)).sum(2)
            # 在bbox_decode函数中
            # print("anchor_points shape:", anchor_points.shape)
            # print("pred_dist shape:", pred_dist.shape)
        return dist2bbox(pred_dist, anchor_points, xywh=False)

    def __call__(self, p, targets, img=None, epoch=0, da_loss=None):
        loss = torch.zeros(4, device=self.device)  # box, cls, dfl, da_loss
        feats = p[1][0] if isinstance(p, tuple) else p[0]
        feats2 = p[1][1] if isinstance(p, tuple) else p[1]
        ...
        # # 打印权重计算后的损失值
        # print(f"Box Loss (weighted): {loss[0].item():.4f}, "
        #       f"Classification Loss (weighted): {loss[1].item():.4f}, "
        #       f"DFL Loss (weighted): {loss[2].item():.4f}, "
        #       f"Domain Adaptation Loss (weighted): {loss[3].item():.4f}")
