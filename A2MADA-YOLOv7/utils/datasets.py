...
from damei.data import augment
...
def create_target_dataloader(path, imgsz, batch_size, stride, opt, cache=False, pad=0.0, rect=False, rank=-1, world_size=1, workers=8, quad=False, prefix=''
                             , hyp=None,augment=False):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesWithoutLabels(path, imgsz, batch_size,
                                          augment=augment,  # augment images
                                          hyp=hyp,  # augmentation hyperparameters
                                          rect=rect,  # rectangular training
                                          cache_images=cache,
    ...
    batch_size = min(batch_size, len(dataset))
    nw = min([os.cpu_count() // world_size, batch_size if batch_size > 1 else 0, workers])  # 工作线程数
    sampler = torch.utils.data.distributed.DistributedSampler(dataset) if rank != -1 else None
    dataloader = torch.utils.data.DataLoader(dataset,
                                             ...
                                             sampler=sampler,
                                             pin_memory=True,
                                             collate_fn=LoadImagesWithoutLabels.collate_fn)
    return dataloader, dataset
...
class LoadImagesWithoutLabels(Dataset):  # for training/testing without labels
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        pbar = tqdm(self.img_files, desc='Scanning images', total=len(self.img_files))
        for im_file in pbar:
            try:
                # verify images
                im = Image.open(im_file)
