...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs, da_loss=0):  # predictions, targets, model
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        ...
        loss = lbox + lobj + lcls + 0.1 * da_loss
        da_loss = 0.1 * da_loss
        return loss * bs, torch.cat((lbox, lobj, lcls, da_loss, loss)).detach()
